...
internal sealed class ChatStreamingCoordinator(
    ...
    RedisCancellationManager cancellationManager) : IChatStreamingCoordinator
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var fragment in stream.WithCancellation(cancellationToken))
        {
            ...
            if (fragment.IsFinal && fragment.Sender == ChatRole.Assistant.Value)
            {
                yield break;
            }
        }
    ...
    private async Task TryAsync(Func<Task> action, [CallerMemberName] string caller = "")
    {
        try
        {
            await action();
